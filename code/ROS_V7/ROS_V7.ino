...
//Wheel distance in meters for one encoder pulse
#define ONE_PULSE 0.0006109
...
int motorSpeedValueRight=0; //current speed
float desiredVelocityRight=0; //wanted speed
...
//Serial.begin(57600);
  ...
  attachInterrupt(digitalPinToInterrupt(12), updateLeftVelocity, RISING);
    pinMode(13, INPUT_PULLDOWN);
  attachInterrupt(digitalPinToInterrupt(13), updateRightVelocity, RISING);
...
*/
 ...
 updateVelocity();
  ...
  //rightMotorControl(0.1,speed_ang);
 // leftMotorControl(0.1,speed_ang);
...
unsigned long leftPrev=micros();
unsigned long leftTimeElapsed=1;
unsigned long rightPrev=micros();
unsigned long rightTimeElapsed=1;

void updateRightVelocity(void){
    rightTimeElapsed=micros()-rightPrev; 
    rightPrev=micros();
    //Serial.println(rightTimeElapsed);
}

void updateLeftVelocity(void){
    leftTimeElapsed=micros()-leftPrev; 
    leftPrev=micros();
}
  
void updateVelocity(void){

  leftVelocity  = ONE_PULSE / (leftTimeElapsed  * 0.000001);
  rightVelocity = ONE_PULSE / (rightTimeElapsed * 0.000001);
